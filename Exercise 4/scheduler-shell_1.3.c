...
void push(struct Queue *q, int id, int pid, char exe [],char priority, int is_shell) {
	q->size++;
...
void print_queue(struct Queue *q){
	...
	while (tmp != NULL) {
		printf("%d (pid: %d, name: %s, priority: %c) --> ", tmp->id,tmp->pid,tmp->name,tmp->priority);
		tmp = tmp->next;
	}
	printf("NULL\n");
}
...
sched_create_task(char *executable)
{
	...
	if(p < 0){
		...
		push(&q,next_id(&q),p,executable,'l',0); //Scheduler pushes child's pid to queue
		increase_next_id(&q);
	}
}
...
sched_high_task(int search_id)
{
	...
	if(tmp->priority == 'l') {
		(&q)->has_high++;
		printf("Scheduler: Promoting (id:%d, pid:%d, name:%s, priority:%c) to high priority.\n",
				tmp->id,tmp->pid,tmp->name,tmp->priority);
		tmp->priority = 'h';
	}
}
...
sched_low_task(int search_id)
{
	...
	if(tmp->priority == 'h') {
		(&q)->has_high--;
		printf("Scheduler: Demoting (id:%d, pid:%d, name:%s, priority:%c) to low priority.\n",
				tmp->id,tmp->pid,tmp->name,tmp->priority);
		tmp->priority = 'l';
	}
}
...
sigalrm_handler(int signum)
{
	...
	printf("Scheduler: Stopping (id:%d, pid:%d, name:%s, priority:%c).\n",
			front(&q)->id,front(&q)->pid,front(&q)->name,front(&q)->priority);
...
sigchld_handler(int signum)
{
	...
	for (;;) {
		...
		if (WIFEXITED(status)) {
			printf("Scheduler: (id:%d, pid:%d, name:%s, priority:%c) terminated normally.\n",
					find_node_from_pid(&q,p)->id,find_node_from_pid(&q,p)->pid,
					find_node_from_pid(&q,p)->name,find_node_from_pid(&q,p)->priority);
			pop(&q); // remove child from queue
		}
		if(WIFSIGNALED(status)){
			//a child has died by signal
			printf("Scheduler: (id:%d, pid:%d, name:%s, priority:%c) has been killed.\n",
					find_node_from_pid(&q,p)->id,find_node_from_pid(&q,p)->pid,
					find_node_from_pid(&q,p)->name,find_node_from_pid(&q,p)->priority);
			//if(p != (front(&q)->pid)){
